...
const TvSeriesPage = ({ tvSeries: initialTvSeries }) => {
  const [tvSeries, setTvSeries] = useState(initialTvSeries);

  const handleSearch = (searchTerm) => {
    const filteredTvSeries = initialTvSeries.filter(tvSeries =>
      tvSeries.name && tvSeries.name.toLowerCase().includes(searchTerm.toLowerCase())
    );
        ...
        {tvSeries && tvSeries.length > 0 && (
          <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {tvSeries.map(tvSeries => (
              <div key={tvSeries.id} className="bg-white rounded-3xl shadow-xl">
                ...
                <div className="p-6">
                  <a href={`#`} className="font-bold text-xl line-clamp-2 hover:text-cyan-700">
                    {tvSeries.name}
                  </a>
                  <span className="text-slate-400 pt-2 font-semibold">
                    ({tvSeries.year})
                  </span>
                  <p className="text-gray-700 py-2 line-clamp-4">{tvSeries.overview}</p>
...
export async function getServerSideProps() {
  try {
    const res = await fetch('https://api.themoviedb.org/3/tv/popular?api_key=dab11bb1b0e0ea35a5fd7a3a44dd8f1c'); 
    const data = await res.json();

    const tvSeries = data.results || []; 

    return { props: { tvSeries } };  } catch (error) {
